...
args = parser.parse_args(['./data/cifar.python', '--dataset', 'cifar10', '--arch', 'resnet20',
                         '--save_path', './logs/cifar10_resnet110_norm2_0_324_3_rate0.7', '--epochs', '200',
                        '--batch_size', '16', '--layer_end', '57', '--evaluate','--resume','model_best.pth-test.tar'])
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # 使用线性退火方式
        m.decay_rate = max(0,float('%.4f' % (decay_rate_init * (1 - (2*epoch+1) / args.epochs))))
...
def save_checkpoint(state, is_best, save_path, filename):
    ...
    if is_best:
        bestname = os.path.join(save_path, 'model_best.pth-test.tar')
...
class Mask:
    ...
    def get_filter_similar(self, weight_torch, compress_rate, distance_rate, length, dist_type="l2"):
        codebook = np.ones(length)
        if len(weight_torch.size()) == 4:
            filter_pruned_num = int(weight_torch.size()[0] * (1 - compress_rate))  # 共需要被剪枝的数量
            similar_pruned_num = int(weight_torch.size()[0] * distance_rate)  # 采用距离规则被剪枝的数量
            weight_vec = weight_torch.view(weight_torch.size()[0], -1)  # 将该层的卷积核全部拉成一维向量(in_channel,kernal_size)

            if dist_type == "l2" or "cos":
                norm = torch.norm(weight_vec, 2, 1)  # 计算一维卷积核的L2范数(in_channel,kernal_size)
            ...
            filter_small_index = []
            filter_large_index = []
            filter_large_index = norm_np.argsort()[filter_pruned_num:]  # 进行排序获得值高的部分(reserve,kernal_size)
            filter_small_index = norm_np.argsort()[:filter_pruned_num]  # 获得值低的部分
